...
//
//   第一层  enable.skills / mcp / extensions / packages + 设置 + 原始 JSON + 保存
...
async function toggleGroup(
  ...
  while (true) {
    ...
    const action = await showPersistentShortcutMenu(
      ...
      [],
      {
        getContext: () => `已启用 ${enabledCount} / ${ids.length} 项`,
...
async function editSettingsSection(
  ...
  while (true) {
    ...
    const action = await showPersistentFormMenu(ctx, `${title} › 设置`, "", rows, cursor, {
      ...
      getDetailLines: (row) => {
        const help = row ? FIELD_HELP[row.id] : undefined;
        return help ? [`  ${help}`] : [];
      },
      hints: [
        { key: "↑↓", label: "选择" },
  ...
  while (true) {
    const rows: MenuRow[] = [
      ...RESOURCE_GROUPS.map((group) => ({
        ...
        searchText: `enable.${group}`,
      })),
      { id: "settings", label: `${padLabel("设置", 24)}${describeSettingsSummary(preset, base)}`, searchText: "设置 settings" },
      { id: "settingsRaw", label: `${padLabel("原始 JSON", 24)}编辑全部设置`, searchText: "原始 JSON settingsRaw" },
      { id: "save", label: `${padLabel("保存", 24)}写入 presets.yml`, searchText: "保存 save" },
    ];
    const action = await showPersistentFormMenu(ctx, title, "", rows, cursor, {
      ...
      getDetailLines: (row) => {
        ...
        if (row.id.startsWith("group:")) {
          ...
          return [`  Enter 进入 ${group} 成员列表，逐项切换启用状态。`];
        }
        if (row.id === "settings") return ["  进入后编辑 defaultProvider / defaultModel / defaultThinkingLevel / tools。"];
        if (row.id === "settingsRaw") return ["  直接编辑 settings 段的 JSON，保存后整体替换。"];
        if (row.id === "save") return ["  写入 presets.yml；写入前做 schema 校验。"];
        return [];
      },
      hints: [
        { key: "↑↓", label: "选择" },
    ...
    if (action.id.startsWith("group:")) {
      await toggleGroup(ctx, action.id.slice("group:".length) as ResourceGroup, preset, config, base);
      continue;
    }
  }
}
...
export async function editResourceGroup(
  ...
  while (true) {
    ...
    const action = await showPersistentShortcutMenu<"new" | "delete">(
      ...
      {
        getContext: () => `${ids.length} 项 · ${objectMode ? "对象注册表" : "ID 列表"}`,
        getDetailLines: (row) => {
          if (!row || row.id === NEW_ROW) return [`  注册表决定 preset 的 enable.${group} 里能选哪些 ID。`];
