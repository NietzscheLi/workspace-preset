...
2. 应用 settings：`defaultProvider` + `defaultModel`（需同时设置，找不到模型或缺认证即报错）、`defaultThinkingLevel`、`tools`（工具必须存在）；
3. 写 `preset-mcp.json` 快照（校验 `mcp-registry.json`）；
4. 写项目 `.pi/preset.json`；
5. 更新状态栏键 `preset`（Base 时为 `undefined`）；
6. 若激活项与上一次不同，调用 `ctx.reload()` 让 skills / MCP / extensions / packages 重新加载；激活项未变时只提示 `Preset activated: <name>`、不 reload。
